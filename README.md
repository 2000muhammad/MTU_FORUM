...
| `TELEGRAM_BOT_USERNAME` | имя Telegram-бота без `@`; необязательно, если оно определяется по токену |
...
На старой и новой страницах входа пользователь может восстановить пароль через Telegram. Система создаёт одноразовую ссылку на 10 минут и выдаёт новый случайный пароль только тому Telegram-аккаунту, который пользователь подтвердил в профиле отправкой своего контакта. Для работы нужны `TELEGRAM_BOT_TOKEN`, одинаковый `TELEGRAM_API_KEY` у Django и бота, а также запущенный процесс `telegram_bot/main.py`.

Защита авторизации ограничивает восстановление пароля тремя запросами за 15 минут и блокирует вход на 15 минут после пяти неверных паролей. Все ошибки и блокировки записываются в журнал безопасности.

При `AUTO_MAINTENANCE_ENABLED=1` приложение один раз в сутки создаёт JSON-копию базы в `backups/`, удаляет копии старше `BACKUP_RETENTION_DAYS`, проверяет Telegram-бота и активные внешние API. Ту же проверку можно запустить вручную: `python manage.py automated_maintenance`.
